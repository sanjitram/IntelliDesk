...
export function CustomerSidebar({ ticket }: { ticket: Ticket }) {
  ...
  const getTierInfo = (tier: string) => {
    ...
    if (t.includes("enterprise") || t.includes("platinum")) {
        ...
        return { icon: Shield, color: "text-muted-foreground", bg: "bg-secondary", label: "Standard" };
    }
  };
    ...
    >
      ...
      {isOpen ? (
        ...
        <div className="flex flex-col gap-6 fade-in animate-in duration-300">
          <div>
            <h3 className="text-xs font-bold text-muted-foreground uppercase mb-3">Customer Intelligence</h3>
            <div className="bg-card/50 p-3 rounded-lg border border-border shadow-sm flex flex-col gap-3 transition-colors">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold text-foreground leading-tight">{customer.company}</div>
                   <a href={`https://${customer.domain}`} target="_blank" className="text-xs text-primary/80 hover:text-primary hover:underline flex items-center gap-1">
              ...
              <div className="flex items-center gap-2 text-sm pt-2 border-t border-border">
                ...
                <span className="font-semibold text-foreground">{customer.tier}</span>
              </div>
            </div>
          </div>

           <div>
            <h3 className="text-xs font-bold text-muted-foreground uppercase mb-3">Context</h3>
            <div className="flex flex-col gap-3">
                 <div className="flex items-start gap-2 text-sm">
                    <Users className="w-4 h-4 text-muted-foreground mt-0.5" />
                    <div>
                        <div className="text-muted-foreground text-xs">Assigned CSM</div>
                        <div className="font-medium text-foreground">{customer.csm}</div>
                    </div>
                </div>
                ...
                <div className="flex items-start gap-2 text-sm">
                    ...
                    <div>
                        ...
                        <div className="font-medium text-foreground">{customer.role}</div>
                    </div>
                </div>
                 ...
                 <div className="flex items-start gap-2 text-sm">
                    ...
                    <div>
                        <div className="text-muted-foreground text-xs">Latest Deal</div>
                        <div className="font-medium text-foreground">12,500 <span className="text-green-600 text-xs">(Open)</span></div>
                    </div>
                </div>
            </div>
          </div>
        ...
        <div className="flex flex-col items-center gap-4 animate-in fade-in duration-300 cursor-pointer" onClick={() => setIsOpen(true)}>
             <div className="p-2 rounded-lg bg-secondary" title={customer.company}>
                <Building2 className="w-5 h-5 text-muted-foreground" />
             </div>
